...
ShipOptions::ShipOptions(QWidget *parent) : QDialog(parent)
{
    setupUi();
}

void ShipOptions::okeyClick()
{
    bool s0, s1, s2, s3, s4, s5;
    float q1, q2, q3, q4, q5;

    s0 = radio_euler->isChecked();
    q1 = edt_b->text().toFloat(&s1);
    q2 = edt_err->text().toFloat(&s2);
    q3 = edt_x0->text().toFloat(&s3);
    q4 = edt_y0->text().toFloat(&s4);
    q5 = edt_t0->text().toFloat(&s5);
    ...
    {
        emit this->setParam(s0, q1, q2, q3, q4, q5);
        emit this->accept();
    }
    else
    {
...
void ShipOptions::setupUi()
{
    ...
    pixmap->setPixmap(QPixmap(":/images/ship.png"));
    ...
    QLabel *lbl_b = new QLabel(tr("Coefficient beta:"));
    ...
    layout_param->addWidget(lbl_b, 0, 0);
    layout_param->addWidget(edt_b, 0, 1);
    layout_param->addWidget(lbl_err, 1, 0);
    layout_param->addWidget(edt_err, 1, 1);
