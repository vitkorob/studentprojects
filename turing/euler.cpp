...
void Euler::nextValue(double *x, double *y, double h)
{
    *x += h * p(*x, *y);
    *y += h * q(*x, *y);
}

double Euler::p(double x2, double y2)
{
    return A + x2 * x2 * y2 - (B + 1) * x2;
}

double Euler::q(double x2, double y2)
{
...
void Euler::run()
{
    ...
    while(next_t0 < t)
    {
        ...
        if(fabs(next_x0 - next_x2) <= epsilon && fabs(next_x3 - next_x1) > epsilon)
        {
        ...
        else if(fabs(next_x3 - next_x1) <= epsilon)
        {
            step *= 2;
        }
        else
